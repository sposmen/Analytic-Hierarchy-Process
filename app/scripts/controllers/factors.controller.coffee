...
($scope, $rootScope, Factor) ->
  ...
  pairwiseChange=->
    ...
    for row,i in $scope.parentFactor.pairwise
      $scope.parentFactor.pairwise[i][i] = "1" unless $scope.parentFactor.pairwise[i][i]?
      for column,j in row
    ...
    for v1, i in $scope.factors
      ...
      for v2, j in $scope.factors
        $scope.parentFactor.columnSumUnit[j] = 1 if $scope.parentFactor.columnSumUnit[j] is undefined
  ...
  pair_wise_optionsChange=->
    for factor,k in $scope.factors 
      ...
      for row,i in $scope.parentFactor.pair_wise_options[k]
        $scope.parentFactor.pair_wise_options[k][i][i] = "1" unless $scope.parentFactor.pair_wise_options[k][i][i]?
        for column,j in row
          $scope.parentFactor.pair_wise_options[k][j][i] = "1/(" + column + ")" if i < j
      ...
      for v1, i in $scope.options
        ...
        for v2, j in $scope.options
          $scope.parentFactor.columnSumUnitOptions[k][j] = 1 if $scope.parentFactor.columnSumUnitOptions[k][j] is undefined
  ...
  $scope.$watch 'options', (newValue, oldValue) ->
    $scope.parentFactor.pair_wise_options = []
